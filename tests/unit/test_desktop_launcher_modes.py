...
def _read_first(*installed_then_relative):
    """Read a config file, preferring installed copies over the checkout.

    All arguments but the last are absolute paths inside the image; the last is
    the path within the repository.
    """
    *installed, relative = installed_then_relative
    return resolve_source(installed, relative).read_text(encoding="utf-8")


def _firefox_wrapper_path():
    return resolve_source(
        "/usr/local/bin/neurodesktop-firefox", "config/firefox/neurodesktop-firefox"
    )
